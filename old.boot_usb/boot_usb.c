 ...
 * Patched 2010 JCD - Changed chunck size to 4096, resolved bogus error message.
 *
...
static int qt2410_send_file(u_int32_t addr, void *data, u_int32_t len)
{
	...
	for (cur = buf; cur < buf+len_total; cur += CHUNK_SIZE) {
		...
		ret = usb_bulk_write(hdl, QT2410_OUT_EP, cur, remain, usb_timeout_ms);
...
int main(int argc, char **argv)
{
	...
	ret = qt2410_send_file(KERNEL_RAM_BASE, prog, st.st_size);
	if ( ret < 0) {
		printf("Error downloading program\n");
		exit(1);
	}
	printf("Downloading complete, no errors\n");
	exit(0);
}
